...
@Component({
  selector: 'app-change-translate-value',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './change-translate-value.component.html',
  styleUrl: './change-translate-value.component.scss',
  providers: [SecurityService,PageService,SortService,FilterService, FreezeService, ResizeService]
})

export class ChangeTranslateValueComponent  implements OnInit, OnDestroy {
  collapsed = false;
  ...
  async getLanguageList() {
    ...
    this.securityService.getLanguageList(this.payload).then(res => {
      let result : any = res;
  ...
  reset(data) {
    const messagebody = this.translate.instant('DialogReset');
    const isConfirmed = this.utils.dialogMessageAll('confirm', messagebody);
    isConfirmed.then(async (res) => {
      if (res) {
        let res = await this.securityService.resetLanguage(data.key);
        if (res.status) {
  ...
  edit(data) {
    this.router.navigate([data.key], { relativeTo: this.routed });
  }
