...
export class TeamAssignmentComponent implements OnInit {
  ...
  async ngOnInit() {
    this.customAttributes = { class: 'customcss' };
      ...
      this.permission = await this.utils.getPermission('0403');
  ...
  async getList() {
    ...
    const result = await this.collectorService.getTeamAssignmentPaged(this.payload);
...
updateHeaderCheckboxState() {
  ...
  async delete(data: any = null) {
    ...
    if (isConfirmed) {
      try {
        ...
        // console.log(targets)
        const res: any = await this.collectorService.deleteTeamAssignment(targets);
        if (res.status) {
  ...
  detail(item: any) {
    this.router.navigate([item.assignmentId], { relativeTo: this.routed });
  }
  ...
  edit(data: any) {
    this.router.navigate([data.assignmentId], { relativeTo: this.routed });
  }
